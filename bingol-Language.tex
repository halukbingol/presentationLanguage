...
\begin{frame}\frametitle{Protolanguage}
    	Goal:
	Agent $i$ wants to transfer a meaning to agent $j$
...
\begin{frame}\frametitle{Naming}
	...
	\begin{column}{.5\linewidth}   
		...
		\begin{itemize}
			...
			\begin{itemize}
				\item 
				obtaining meaning-symbol associations
			\end{itemize}
		\end{itemize}
	\end{column}
	\begin{column}{.5\linewidth}
...
\newcommand{\TwoPersonLanguage}{
		...
		\begin{itemize}
			...
			\item
			\hbDefined{comprehension} from $i$ to $j$\\
			$
				\hbDefined{F(i \to j)} 
				\triangleq
					\frac{1}{M}
			...
			\item
			\hbDefined{mutual comprehension} from $i$ to $j$\\
			$
...
\begin{frame}\frametitle{Model}
	...
	\begin{column}{.5\linewidth}
		...
		\begin{itemize}
			...
			\item
			\hbDefined{within community comprehension} for $\hbSet{C} \subseteq \hbSet{P}$\\
			$
				\hbDefined{W(\hbSet{C})} 
				= 	
...
\begin{frame}\frametitle{Evolutionary Dynamics}
	...
	\begin{column}{.5\linewidth}
		\includegraphics[width=0.5\columnwidth]%
			{Fig-stickmanCommunity}\\
		\qquad \hbRemark{Generation $G_{i+1}$}
	\end{column}
	\end{columns}
...
%	\hbEmph{
%		Assumption:
%		Child learns from single teacher.
%	}\\
